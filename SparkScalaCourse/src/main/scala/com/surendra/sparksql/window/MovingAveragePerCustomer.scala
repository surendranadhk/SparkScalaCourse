package com.surendra.sparksql.window
...
object MovingAveragePerCustomer {
    ...
    def main(args: Array[String]) {
      ...
      val windowByCategoryTimeAsc = Window.partitionBy('cc_num).orderBy('trans_time).rowsBetween(-1, 1)

      val movingAvgDF = joinedDF.withColumn( "movingAvg", avg(transactionDF("amt")).over(windowByCategoryTimeAsc))

      movingAvgDF.show(false)
